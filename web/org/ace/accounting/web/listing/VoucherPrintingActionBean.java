...
public class VoucherPrintingActionBean extends BaseBean {
	...
	public void init() {
		chkVoucherNo = false;
		ftdto = new VPFTdto();
		ftdto.setStartDate(startDate);

		tidto = new VPFTdto();
		tidto.setEndDate(endDate);
	...
	public void chkClick() {
		if (startDate != null && endDate != null) {
			...
			voucherNoList.clear();
			voucherNoList = listingReportService.findFromVoucherNo(startDate, endDate, branch);
	...
	public void searchResults() {
		if (validate()) {
			if (!chkVoucherNo) {
				dtoList = listingReportService.findVoucherPrinting(startDate, endDate);
				if (dtoList.size() == 0) {
				...
				dtoList = listingReportService.findVoucherPrintingByVoucherNo(startDate, endDate, ftdto.getVoucherNo(), tidto.getVoucherNo());
				if (dtoList.size() == 0) {
					addErrorMessage(null, MessageId.NO_RESULT);
				} else {
					generateReport(dtoList);
